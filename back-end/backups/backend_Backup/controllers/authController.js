...
const login = async (req, res) => {
  try {
    ...
    // 1. ค้นหา User ในฐานข้อมูลว่ามีชื่อนี้ไหม และยังใช้งานได้อยู่ (is_active = 1)
...
const getMe = async (req, res) => {
  try {
    ...
    const [rows] = await db.execute(
      "SELECT id, username, full_name, role, created_at FROM users WHERE id = ? AND is_active = 1",
      [req.user.id]
    );
