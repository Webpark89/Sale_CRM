  ...
  return (
    <aside 
      style={{ 
        ...
        padding: isSidebarExpanded ? "24px 16px" : "24px 8px", 
        display: "flex", 
    ...
    >
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 4, flex: 1 }}>
        {isSidebarExpanded && <div style={{ fontSize: 11, fontWeight: 700, fontFamily: RG.fontHeading, color: RG.textMuted, letterSpacing: 1, marginBottom: 8, paddingLeft: 12, textAlign: "left", opacity: isSidebarExpanded ? 1 : 0.8, transition: "all 0.3s" }}>MENU</div>}
        {navItems.map(n => (
          <button key={n.key} onClick={() => navigate("/" + n.key)} style={{ position: "relative", padding: isSidebarExpanded ? "12px 16px" : "12px 0", borderRadius: 8, border: "none", background: page === n.key ? RG.sidebarActiveBg : "transparent", color: page === n.key ? RG.sidebarTextActive : RG.sidebarText, cursor: "pointer", fontWeight: page === n.key ? 600 : 500, fontSize: 14, fontFamily: RG.fontHeading, transition: "all 0.2s", display: "flex", alignItems: "center", justifyContent: isSidebarExpanded ? "flex-start" : "center", gap: isSidebarExpanded ? 12 : 0, boxShadow: "none", whiteSpace: "nowrap", marginBottom: 2 }}>
            <span style={{ fontSize: 18, width: 24, display: "flex", justifyContent: "center", opacity: page === n.key ? 1 : 0.7 }}>{n.icon}</span> 
